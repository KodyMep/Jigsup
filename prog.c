...
Process* RR(Queue* queue, int quantum) {
    ...
    // Create a copy of the next_process
    Process* modified_process = create_process(
        next_process->PID, next_process->PR, next_process->numCPUBurst, next_process->numIOBurst,
        next_process->CPUBurst, next_process->IOBurst
    );
    modified_process->cpuindex = next_process->cpuindex;
    modified_process->remaining_burst_time = next_process->remaining_burst_time;

    // Decrease the remaining burst time of the process by the quantum
    modified_process->remaining_burst_time -= quantum;

    // If the burst time of the process is less than or equal to 0, the process is done
    if (modified_process->remaining_burst_time <= 0) {
        // The process has completed its current CPU burst
        modified_process->cpuindex++;
        modified_process->remaining_burst_time = modified_process->CPUBurst[modified_process->cpuindex];
    } else {
        // The process still has some burst time remaining, so move it to the end of the queue
        queue_enqueue(queue, modified_process);
    }
